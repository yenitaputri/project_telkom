                ...
                <tbody class="text-gray-600">
                    @foreach ($astinets as $index => $astinet)
                        <tr class="border-t hover:bg-gray-100 transition">
                            ...
                            <td class="px-2 py-2 text-left align-middle">{{ $astinet->nama_sales }}</td>
...
<td class="px-2 py-2 text-center align-middle space-x-1">
    ...
    {{-- Tombol Hapus --}}
    <form action="{{ route('astinet.destroy', $astinet->id) }}" method="POST" class="inline-block" id="delete-form-{{ $astinet->id }}">
        @csrf
        @method('DELETE')
        <button type="button"
