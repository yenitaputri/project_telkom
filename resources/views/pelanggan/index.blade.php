...
<div class="bg-white rounded-lg shadow-md p-6 min-h-[calc(100vh-160px)] flex flex-col">
    ...
    <div class="flex justify-end mb-4 space-x-4 items-center" x-data="{ openTambahModal: false }">
        ...
        <button @click="openTambahModal = true" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg flex items-center transition-colors duration-200 inline-flex">
        ...
        <!-- Modal Tambah Data -->
        <div x-show="openTambahModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4" x-cloak>
            <div @click.away="openTambahModal = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-auto relative">
                <h3 class="text-xl font-bold mb-4 text-center">Tambah</h3>
                <form action="#" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
                        <label for="file_upload" class="block text-gray-700 font-semibold mb-2">Unggah Data Pelanggan</label>
                        <input type="file" id="file_upload" name="file_upload" class="w-full border border-gray-300 rounded px-3 py-2" />
                        <p class="text-red-600 text-sm mt-1">*Unggah file dengan format Excel (.xls atau .xlsx)</p>
                    </div>
                    <div class="flex justify-end space-x-4">
                        <button type="button" @click="openTambahModal = false" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded transition-colors duration-200">
                            Batal
                        </button>
